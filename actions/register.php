...
if(empty($errors)){
    $sql = "INSERT INTO users SET firstname = ?, `name` = ?, username = ?, password = ?, mail = ?, levelUser = 1";
    $req = $dbh->prepare($sql);
    $password = password_hash($_POST['password'], PASSWORD_BCRYPT);
    $req->execute([$_POST['name'], $_POST['firstname'], $_POST['username'], $password, $_POST['mail']]);
    die('<div class="ui container center aligned segment">
            <h2 class="ui header center aligned">Votre inscription s\'est bien déroulée</h2>
            <h4 class="ui header center aligned"><a href="?page=accueil">Retour a l\'accueil</a>
         </div>');
}
